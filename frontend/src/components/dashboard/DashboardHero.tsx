import { ArrowRight, Radar, Database, Cpu, Code, GitBranch, Zap, Layers, FileCode, Network, Bug, Shield, Activity, BarChart3, CpuIcon } from "lucide-react";
  ...
  return (
    <Panel className="bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 border border-slate-800/30 shadow-2xl">
      <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          {/* 系统状态标识 - 精致版 */}
          <div className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-emerald-500/10 to-cyan-500/10 border border-emerald-500/20 px-4 py-2 mb-8">
            <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></div>
            <p className="text-xs font-medium uppercase tracking-[0.15em] text-emerald-400">SYSTEM STATUS: OPERATIONAL</p>
          ...
          <h1 className="text-4xl font-bold leading-tight lg:text-5xl tracking-tight">
            ...
            <span className="block text-slate-300 mt-3">企业级安全态势感知系统</span>
          </h1>
          
          <p className="mt-6 max-w-2xl text-base leading-7 text-slate-400">
            基于 LibHunter 跨内联匹配与 PHunter 切片分析引擎，实现 APK 漏洞自动化验证、供应链风险检测、语义补丁适配与实时威胁情报聚合。
          </p>
          ...
          <div className="mt-10">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div 
                  key={stat.title}
                  className={`rounded-xl ${stat.bg} ${stat.border} border backdrop-blur-sm p-4 transition-all duration-300 hover:scale-[1.02] hover:shadow-lg hover:shadow-current/10`}
                >
        ...
        <div className="space-y-6">
          <div className="rounded-xl bg-gradient-to-br from-slate-900/60 to-slate-950/60 border border-slate-800/50 backdrop-blur-sm p-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div 
                key={item.title}
                className="rounded-xl bg-slate-900/40 border border-slate-800/40 backdrop-blur-sm p-4 transition-all duration-300 hover:bg-slate-800/50 hover:scale-[1.02] hover:shadow-md"
              >
                <div className="flex items-center gap-2 mb-2">
                  {item.icon}
                  <span className="text-xs font-medium text-white">{item.title}</span>
                </div>
                <div className={`text-xl font-bold ${item.color} font-mono`}>{item.value.toLocaleString()}</div>
                <div className="text-xs text-slate-400 mt-1">{item.desc}</div>
                <div className="h-px w-full bg-gradient-to-r from-transparent via-current to-transparent opacity-20 mt-2"></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mt-10 pt-6 border-t border-slate-800/50">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></div>
              <span className="text-xs text-slate-400">引擎状态: <span className="text-emerald-400 font-medium">运行中</span></span>
            </div>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-400">数据库: <span className="text-blue-400 font-medium">同步</span></span>
            </div>
          </div>
          <div className="text-xs text-slate-500">
            数据更新时间: <span className="text-slate-300">刚刚</span> • 系统版本: <span className="text-slate-300">v2.4.1</span>
          </div>
        </div>
      </div>
    </Panel>
